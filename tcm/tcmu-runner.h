...
#define FILTER_MAX (unsigned int)16
...
struct tcmu_device {
	...
	unsigned long filter_cnt;
        void (*write_xform[FILTER_MAX])(void* buf, unsigned long cnt, uint64_t offset, bool* doWrite);
        void (*read_xform[FILTER_MAX])(void* buf, unsigned long cnt, uint64_t offset);
	char *(*get_filter_name[FILTER_MAX])(void);
	//void (*prefetch[FILTER_MAX])(unsigned long *cnt, uint64_t *offset);
	int (*pre_read[FILTER_MAX])(void *buf,unsigned long cnt, uint64_t offset, unsigned int fdesc, bool *doRead);
	int (*pass_args[FILTER_MAX])(char* raw_arg_list);
};
